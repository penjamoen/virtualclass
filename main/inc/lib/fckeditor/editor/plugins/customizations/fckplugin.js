...
if ( !FCKConfig.ImagesIcon )
{
	...
	FCKConfig.ImagesIcon = FCKConfig.PluginsPath + 'customizations/images/images_icon.png' ;
}

// The icon for the link properties button/command.
if ( !FCKConfig.LinkIcon )
{
	// This is the original icon from a chosen skin.
	//FCKConfig.ImagesIcon = 34 ;
	// We will use a better icon by default.
	FCKConfig.LinkIcon = FCKConfig.PluginsPath + 'customizations/images/link_icon.png' ;
}

// The icon for the unlink properties button/command.
if ( !FCKConfig.UnlinkIcon )
{
	// This is the original icon from a chosen skin.
	//FCKConfig.ImagesIcon = 35 ;
	// We will use a better icon by default.
	FCKConfig.UnlinkIcon = FCKConfig.PluginsPath + 'customizations/images/unlink_icon.png' ;
}

// The icon for the pasteword properties button/command.
if ( !FCKConfig.PastewordIcon )
{
	// This is the original icon from a chosen skin.
	//FCKConfig.ImagesIcon = 11 ;
	// We will use a better icon by default.
	FCKConfig.PastewordIcon = FCKConfig.PluginsPath + 'customizations/images/pasteword_icon.png' ;
}

// The icon for the mascoticon properties button/command.
if ( !FCKConfig.MascotIcon )
{
	// This is the original icon from a chosen skin.
	//FCKConfig.ImagesIcon = 37 ;
	// We will use a better icon by default.
	FCKConfig.MascotIcon = FCKConfig.PluginsPath + 'customizations/images/mascot_icon.png' ;
}
...
FCKToolbarItems.GetItem = function( itemName )
{
	...
	switch ( itemName )
	{
		...
		case 'Mascot'			: oItem = new FCKToolbarButton( 'Mascot'		, FCKLang.InsertImageLbl, FCKLang.InsertImage, null, false, true, FCKConfig.MascotIcon ) ; break ;
...
FCK.ContextMenu.RegisterListener( {
	AddItems : function ( menu, tag, tagName )
	{
		...
		if ( FCKLang.DlgAudioTitle )
		{
			...
			AudioIcon = FCKConfig.PluginsPath + 'audio/audio.png' ;
		}
		...
		if ( tagName == 'IMG' && tag.getAttribute( '_fckvideo' ) )
		{
			switch ( FCK.GetVideoType( tag ) )
			{
				case 'embedded_video' :
					...
					{
						menu.AddSeparator() ;
						menu.AddItem( 'EmbedMovies', FCKLang.DlgEmbedMoviesTitle, FCKConfig.PluginsPath + 'fckEmbedMovies/embedmovies.png' ) ;
					}
					break ;
				case 'youtube' :
