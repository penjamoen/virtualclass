...
if ($origin!='learnpath') {
	...
	echo '<a href="index.php">'.Display::return_icon('go-previous.png',get_lang('BackToForumOverview')).' '.get_lang('BackToForumOverview').'</a>';
		...
		if ($current_forum['locked'] <> 1 AND $current_forum['locked'] <> 1) {
			if (!api_is_anonymous()) {
				if ($my_forum==strval(intval($my_forum))) {
					echo '<a href="newthread.php?'.api_get_cidreq().'&forum='.Security::remove_XSS($my_forum).$origin_string.'">'.Display::return_icon('forumthread_new.png',get_lang('NewTopic')).' '.get_lang('NewTopic').'</a>';
				} else {
					$my_forum=strval(intval($my_forum));
					echo '<a href="newthread.php?'.api_get_cidreq().'&forum='.$my_forum.$origin_string.'">'.Display::return_icon('forumthread_new.png',get_lang('NewTopic')).' '.get_lang('NewTopic').'</a>';
				}
...
if ($origin != 'learnpath') {
  ...
  echo '<div style="background: #DEDEDE;border:solid 2px #ED9438;margin-top:10px;padding-left:35px;"class="rounded">';
...
echo "\t\t<td width='150px'>".get_lang('Title')."</td>\n";
echo "\t\t<td width='50px'>".get_lang('Replies')."</td>\n";
echo "\t\t<td width='50px'>".get_lang('Views')."</td>\n";
echo "\t\t<td width='200px'>".get_lang('Author')."</td>\n";
echo "\t\t<td width='200px'>".get_lang('LastPost')."</td>\n";
echo "\t\t<td width='auto'> ".get_lang('Actions')."</td>\n";
echo "\t</tr>\n";
	...
	foreach ($threads as $row) {
		...
		if (api_is_allowed_to_edit(false,true) OR  !($row['thread_replies']=='0' AND $row['visible']=='0')) {
			...
			if (is_array($my_whatsnew_post_info) and !empty($my_whatsnew_post_info)) {
				echo icon('../img/forumthread_new_22.png');
			} else {
				echo icon('../img/forumthread_new_22.png');
			}
			...
			if ($origin != 'learnpath') {
				if (api_is_allowed_to_edit(false,true) && !(api_is_course_coach() && $current_forum['session_id']!=$_SESSION['id_session'])) {
					echo "<a href=\"editpost.php?".api_get_cidreq()."&forum=".Security::remove_XSS($my_forum)."&amp;thread=".Security::remove_XSS($row['thread_id'])."&amp;post=".$row_post_id['post_id']."&amp;gidReq=".$_SESSION['toolgroup']."&origin=".$origin."&id_attach=".$id_attach."\">".icon('../img/edit_link.png',get_lang('Edit'))."</a>&nbsp;&nbsp;&nbsp;\n";
					echo "<a href=\"".api_get_self()."?".api_get_cidreq()."&forum=".Security::remove_XSS($my_forum)."&amp;action=delete&amp;content=thread&amp;gidReq=".$_SESSION['toolgroup']."&amp;id=".$row['thread_id'].$origin_string."\" onclick=\"javascript:if(!confirm('".addslashes(api_htmlentities(get_lang("DeleteCompleteThread"),ENT_QUOTES,$charset))."')) return false;\">".icon('../img/delete_link.png',get_lang('Delete'))."</a>&nbsp;&nbsp;&nbsp;\n";
					...
					echo '&nbsp;&nbsp;&nbsp;';
					echo "<a href=\"viewforum.php?".api_get_cidreq()."&forum=".Security::remove_XSS($my_forum)."&amp;action=move&amp;gidReq=".$_SESSION['toolgroup']."&amp;thread=".$row['thread_id'].$origin_string."\">".icon('../img/go-next.png',get_lang('MoveThread'))."</a>&nbsp;&nbsp;&nbsp;";
				}
			}
