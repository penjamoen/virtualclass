...
{	
	...
	echo '<script>window.location.href = "admin.php?'.api_get_cidreq().'&editQuestion='.$modifyAnswers.'&fromExercise='.$exerciseId.'&answerType=6&lp_id='.$lp_id.'"</script>';
...
//	Question :: uploadPicture($baseName,$fileName);	
	...
	echo '<script>window.location.href = "'.api_get_self().'?'.api_get_cidReq().'&newQuestion=yes&hotspotadmin='.$insert_id.'&exerciseId='.$exerciseId.'&answerType=6&lp_id='.$lp_id.'"</script>';
...
<?php
 ...
 if($editQuestion != '')
 {
	$action = api_get_self().'?'.'newQuestion=yes&'.api_get_cidReq().'&answerType=6&exerciseId='.$exerciseId.'&hotspotadmin='.$modifyAnswers.'&edit=Y';
 }
 else
 {
	$action = api_get_self().'?'.'newQuestion=yes&'.api_get_cidReq().'&answerType=6&exerciseId='.$exerciseId.'&hotspotadmin='.$modifyAnswers;
 }
?>
 ...
 <form method="post" action="<?php echo $action; ?>" name="frm_exercise" id="frm_exercise" enctype="multipart/form-data">
  ...
  <td width="45%"><input type="file" name="imageUpload"><input type="submit" name="upload" value="Upload" style="background:none;border:0;font-weight:bold;"></td>  
  </tr>
  </table>
...
?>
	 ...
	 <td colspan="2" valign="top"><table class="data_table" style="width:80%;float:right;">
      <tr>
       <th width="">&nbsp;</th>
